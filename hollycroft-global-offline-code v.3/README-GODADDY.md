...
The contact form is set up as a native Netlify Form:
...
2. Go to Forms and confirm the `contact` form appears after deployment.
3. Go to Forms > Form notifications.
4. Add an email notification to `hello@hollycroftglobal.com`.

The form cannot fully submit from a local `file://` preview because Netlify Forms only captures submissions from the deployed Netlify site. Test the form on the published domain.

The form code validates required fields, checks the numerical security question, and uses a hidden spam trap before sending the submission to Netlify.
