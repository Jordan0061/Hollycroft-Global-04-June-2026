...
(() => {
  ...
  document.querySelectorAll("[data-insight-index]").forEach((article) => {
    ...
    readMore.hidden = !insight.expanded;
  ...
  const webhookUrl = typeof content.contactWebhookUrl === "string" ? content.contactWebhookUrl.trim() : "";
  const allowedWebhookPattern = /^(?:contact-submit\.php|https:\/\/script\.google(?:usercontent)?\.com\/)/;
  ...
  const getContactPayload = () => {
    ...
    return {
      ...
      firstName: cleanFormValue(formData.get("first-name"), 80),
      lastName: cleanFormValue(formData.get("last-name"), 80),
      email: cleanFormValue(formData.get("email"), 254),
      phoneCountry: cleanFormValue(formData.get("phone-country"), 20),
      phone: cleanFormValue(formData.get("phone"), 40),
      message: cleanFormValue(formData.get("message"), 5000),
      preferredContactMethod: cleanFormValue(formData.get("preferred-contact-method"), 20)
    };
  };
  ...
  const handleContactSend = async (event) => {
    ...
    if (!webhookUrl) {
      setFormStatus(content.contact?.errorMessage || "Something went wrong. Please email hello@hollycroftglobal.com.");
      return;
    }

    if (isLocalPreview && webhookUrl === "contact-submit.php") {
      setFormStatus(content.contact?.localPreviewMessage || "This form can only send after the site is uploaded to PHP hosting.");
      return;
    }

    if (!allowedWebhookPattern.test(webhookUrl)) {
      setFormStatus(content.contact?.errorMessage || "Something went wrong. Please email hello@hollycroftglobal.com.");
      return;
    }
    ...
    try {
      const isGoogleWebhook = webhookUrl.startsWith("https://script.google");
      const response = await fetch(webhookUrl, {
        method: "POST",
        mode: isGoogleWebhook ? "no-cors" : "same-origin",
        headers: {
      ...
      if (!isGoogleWebhook && !response.ok) throw new Error("Contact request failed");
  ...
  const additionalInsights = [...document.querySelectorAll(".additional-insight")];
