...
(() => {
  ...
  const allowedWebhookHosts = Array.isArray(content.allowedWebhookHosts)
    ? content.allowedWebhookHosts
  ...
  const isAllowedWebhookUrl = (url) => {
    try {
      ...
      return parsedUrl.protocol === "https:" && allowedWebhookHosts.includes(parsedUrl.hostname.toLowerCase());
    } catch {
      return false;
    }
  };
  ...
  const handleContactSend = async (event) => {
    ...
    if (!webhookUrl) {
      setFormStatus(content.contact?.webhookNotConfiguredMessage || "The contact webhook has not been connected yet.");
      return;
    }
    ...
    try {
      ...
      if (response.type !== "opaque" && !response.ok) {
        ...
        throw new Error(serverMessage || content.contact?.endpointErrorMessage || "The contact sender is not responding.");
      }
